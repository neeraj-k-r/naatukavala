...
export default function ReturnRequestForm({
    ...
    return (
      <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50 p-4">
        ...
        {existing.description && (
          <p className="mt-1 text-sm text-slate-600">{existing.description}</p>
        )}
      </div>
    );
  }
  ...
  if (!open) {
    return (
      <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50 p-4">
        ...
        <p className="mt-1 text-xs text-slate-500">{policy}</p>
        <button
          type="button"
  ...
  return (
    ...
    >
      <p className="text-sm font-semibold text-slate-800">Request a return</p>
      <p className="mt-1 text-xs text-slate-500">{policy}</p>
      ...
      <textarea
        ...
        className="mt-2 w-full resize-none rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none focus:border-emerald-500 disabled:opacity-60"
      />
